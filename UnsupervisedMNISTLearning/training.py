...
#Loop through every image randomly
    ...
    #Generate the spike trains for the specific image
        ...
        prog+=1
        #Print the progress
        print('Training progress: (%d / %d) - Elapsed time: %.4f' % (prog, NImages, timeit.default_timer() - start))
        #Add to label number
        ...
        if mode =='test':
            ...
            values,indices = torch.max(network.Activity[:,PatCount],0,keepdim = True)
            if label[0] == classes[indices]: #Shouldn't be indices, should be neuron associated with index
                correct+=1
...
if mode == 'train':
    ...
    plotWeights(RWeights, assignments,network.synapse.wmax,network.synapse.wmin)
    ...
    RWeights = torch.zeros(280,280) #assignments is wrong, as assignments is only from 1 to 10
    ...
    plotConfusion(RWeights,confusion,network.synapse.wmax,network.synapse.wmin)
    print(accuracy)
    print('\n')
    print(OverAccuracy)
    print('\n')
