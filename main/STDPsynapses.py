...
#defines the STDP weight and the STDP weight change. Will be used in conjunction with another object called LIF neuron.
    ...
    #could potentially use a dictionary for the STDP window, or just what you created in MATLAB.
        ...
        self.w = 50e-3*torch.rand(n,784)-25e-3#Initialise random weights, each row represents neuron, each column a different input.
        self.wmin = wmin
        self.wmax = wmax
    ...
    #Potentiate the synaptic weight of neuron Neur, using the DeltaT values.
       #Need to apply the lookup of the STDP window, to produce corresponding current for potentiation.
        ...
        for i in range(len(DeltaTP)):
            DelCurrent[i] = STDPWindow[float(DeltaTP[i])] #Should convert whole tensor to float prior to this for speed increase
    ...
    #Depress the value synaptic weight of neuron Neur, using the values of DeltaT
        DeltaTN = torch.round(DeltaTN*2)/2 #This rounding allows simple implemenation of this specific STDP window.
    ...
    def GetSTDP(self):
        ...
        for i in range(len(b[0,:])):
            key = b[0,i]#Probably don't need conversion, we'll see*1e-3 #Convert to seconds
            val = b[1,i]
            d[key] = val
...
class LIFNeuronGroup:
    ...
    def __init__(self,n,mode,Ve = 0.2,tau = 0.6,R = 100,gamma = 0.01,target = 35,VthMin=0.02,VthMax = 20):
        ...
        self.v = self.Ve*torch.ones_like(torch.Tensor(n)) #Membrane potential at current point in time.
        ...
        self.s = torch.zeros_like(torch.Tensor(n)) #Determine the occurrances of post-synaptic spikes.
