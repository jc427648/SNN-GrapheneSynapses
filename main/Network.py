...
#Define the network. Might need super()
    def __init__(self,n,mode,dt = 0.2e-3):
        ...
        self.group = LIFNeuronGroup(n,mode, Ve = 0.0,tau = 0.1,R = 1000,gamma = 0.005,target = 10,VthMin=0.25,VthMax = 50)
        self.dt = dt
        self.n = n
        ...
        self.CurrCtr = torch.zeros((784)) #Current Counter (not correct, should be a vector)
        self.InhibCtr = torch.zeros((self.n)) #Inhibition counter (May not be needed)
    ...
    #Consider having creating one more file to run the entire MNIST simulation.
        ...
        for t in range(int(time/self.dt)):
            ...
            self.CurrCtr[spikes[t,:]==1] = c_p_w #Spikes is a tensor, not a list.
            #Apply inhibition.

            currentMat = torch.multiply(self.CurrCtr>0,self.synapse.w) #Matrix of all values
            ...
            if torch.sum(self.group.s)>0:
                if (mode == 'train'):
                    ...
                    DeltaT[DeltaT == t] = 80e-3/self.dt#Ensure time is 80ms long (off STDP)

                    DeltaTP,indices = torch.where(DeltaT>0,DeltaT, 400*torch.ones(784,dtype = torch.int)).min(axis=0) #400 should be some variable
                    DeltaTP = 1e3*self.dt*DeltaTP
                    DeltaTN,indices = torch.where(DeltaT<0,DeltaT,-400*torch.ones(784,dtype = torch.int)).max(axis=0)
    ...
    def GetMNIST(self,N,mode):
        ...
        X = X.reshape([N, 784]) #These values are in between 0 and 255, but in should be 20 and 200.Need to fix
        ...
        data = {'X': X, 'y': y } #Store img and lbl in dictionary for reference.
