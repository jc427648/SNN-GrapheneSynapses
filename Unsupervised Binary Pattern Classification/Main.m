...
% Initilise struct matrix with VTEAM model parameters
params = importdata('optimization_params.csv');
params_struct = struct();
params_struct.fieldnames = params.textdata(2:end);
params_struct.data = params.data(2:end);
                    ...
                    for k = 1:num_inputs
                        ...
                        if not(isempty(delta_t))
                            ...
                            for p = 1:2
                                ...
                                if (abs(del_t_value) < 85e-3)
                                    V_ref = [1, 1, 1, 1, 1]; % This should be determined using an additional external function- Vref should be the applied voltage to the given device.
