...
def plotWeights(weights,assignments,maxW,minW,figsize = (10,6)):

    _, axes = plt.subplots(1, 2, figsize=figsize)
		
    color = plt.get_cmap('RdBu', 11)
    ims = axes[0].imshow(weights, cmap='hot_r', vmin=minW, vmax=maxW), axes[1].matshow(assignments, cmap=color, vmin=-1.5, vmax=9.5) 
...
def plotConfusion(weights,confusion,maxW,minW,figsize = (10,6)):
    ...
    color = plt.get_cmap('RdBu', 11)
    ims = axes[0].imshow(weights, cmap='hot_r', vmin=minW, vmax=maxW), axes[1].matshow(confusion, cmap='hot_r', vmin=-0, vmax=torch.max(confusion)) 
    divs = make_axes_locatable(axes[0]), make_axes_locatable(axes[1])
    caxs = divs[0].append_axes("right", size="5%", pad=0.05), divs[1].append_axes("right", size="5%", pad=0.05)
...
def ReshapeWeights(weights,n,assignments = 'none'):
    if (n == 100):
        ...
        for i in range(10):
	        for j in range(28):
		        RWeights[28*i+j,:] = torch.reshape(weights[10*i:10*i+10,28*j:28*j+28],(1,280))
        if assignments != 'none':
        ...
        for i in range(rows):
            for j in range(28):
                RWeights[28*i+j,:] = torch.reshape(weights[cols*i:cols*(i+1),28*j:28*(j+1)],(1,28*cols))
        if assignments!='none':
            assignments = torch.reshape(assignments,(rows,cols))  
    elif (n==50):
        rows = 10
